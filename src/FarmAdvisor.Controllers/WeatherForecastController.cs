...
namespace FarmAdvisor.Controllers
{
    ...
    public class WeatherForecastController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("forecast")]
        public async Task<IActionResult> getForecastAsync([FromQuery] int altitude, double latitude, double longitude)
        {
            try
            {
                ...
                if (altitude > 9000)
                    altitude = 9000;
                List<OneDayWeatherForecast> forecasts = await weatherForecastService.getForecastAsync(altitude, latitude, longitude);
                return Ok(forecasts);
            }
            catch (Exception e)
            {
